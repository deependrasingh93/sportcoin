...
#if defined(NDEBUG)
# error "Sportcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7c4caa8ff68ec626ed87dd6927e501870c8c540d9a21d50d44d00855f005563e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sportcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sportcoin Signed Message:\n";
...
{
    // Sportcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Sportcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Sportcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Sportcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sportcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sportcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7c4caa8ff68ec626ed87dd6927e501870c8c540d9a21d50d44d00855f005563e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The birth of SportCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047a9dfee1912c62375037572ca509d4e5f8a2607c2a727da9173cdc2d4f615d57b183b245d6b4eb5add31e438632bc9479cbe60b177ee232c13f1c163f6899177") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542274914;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086946083;

        if (fTestNet)
        {
            block.nTime    = 1542274924;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4bbfbb6b2339a5879873d5ed7524a2765134cefd8f4fdc244e737d8f5aaf2aa0"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SportcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SportcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SportcoinMiner(CWallet *pwallet)
{
    printf("SportcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sportcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SportcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SportcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SportcoinMiner, pwallet));
}
